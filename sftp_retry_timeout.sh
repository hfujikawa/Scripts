...
if [ $# -ne 1 ] ; then
  echo "Usage:"
  echo "sftp_retry_timeout.sh batchfile"
  exit 1
fi
...
# sftp -o ConnectTimeout=60 guest@192.168.0.10, but not active still 20sec
# also /etc/ssh/ssh_config ConnectTimeout is not active
